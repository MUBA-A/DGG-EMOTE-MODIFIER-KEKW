// ==UserScript==
// @name         Depressing script
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  I'M NOT WEB DEVELOPER. IDK SHIT ABOUT JS | the syntax is "EMOTE :EMOTE" Ex: "OMEGALUL :COGGERS" This will coggers the omegalul
// @author       de_pressed
// @match        https://www.destiny.gg/bigscreen
// @icon         https://www.google.com/s2/favicons?domain=destiny.gg
// @grant        none
// @run-at       document-idle
// ==/UserScript==
//use
const DGG_ANIMATION_EMOTES = [
    "AYAYA", "Blesstiny", "Blade", "COGGERS", "COOMER", "Hhhehhehe",
    "JAMSTINY", "Nappa", "PepeLaugh", "PepoTurkey", "widepeepoHappy", "REE", "MLADY", "flip", "rustle", "weewoo", "wide", "smol", "hyper"
];
const DGG_EMOTES_THAT_WOULD_LOOK_STUPID_WITH_HALLOWEEN_ANIMATION = [
    "GODSTINY","OverRustle",
];
const imgUrl = ' <img style="max-height: 32px; position: relative; margin-top: -32px; top: 7.5px;" src="data:image/jpeg;base64," \> ';

var styleinnerHTML = `
@keyframes rustle {
    from { transform: translateX(0); }
    to { transform: translateX(3px); }
}
@keyframes weewoo {
        0%, 100% {
            opacity: 1;
        }
        10% {
            opacity: 0.5;
            filter: hue-rotate(0deg);
        }
        11% {
            filter: hue-rotate(170deg);
        }
        35% {
            opacity: 1;
        }
        50% {
            opacity: 0.2;
            filter: hue-rotate(170deg);
        }
        51% {
            filter: hue-rotate(310deg);
        }
        65% {
            opacity: 1;
        }
        90%{
            opacity: 0.2;
            filter: hue-rotate(310deg);
        }
        91% {
            filter: hue-rotate(0deg);
        }
    }
._wide {
    transform: scaleX(1.5) scaleY(0.8);
    margin: -30px 15px 0px 15px;
    transition: all 0.2s;
}
._smol {
    transform: scaleX(0.5) scaleY(0.5);
    margin: -15px -10px -15px -10px;
    transition: all 0.2s;
}
._hyper {
  filter: drop-shadow( 0px 0px 3px rgba( 255, 255, 255, 0.7 ) );
  filter:
        opacity( 0.1 )
        drop-shadow( 0px 0px 0px red )
        drop-shadow( 0px 0px 0px red )
        drop-shadow( 0px 0px 0px red )
        drop-shadow( 0px 0px 0px red );
   mix-blend-mode: color-dodge;
}
._rustle {
  animation: rustle 100ms 7;
}
._rustle:hover {
  animation: rustle 100ms infinite;
}
._weewoo {
  animation: weewoo 1.5s 2;
}
._weewoo:hover {
  animation: weewoo 1.5s infinite;
}
._flip {
  transform: scaleX(-1);
}
._AYAYA {
  animation: AYAYA-anim 0.7s 1;
}
._AYAYA:hover {
    animation: AYAYA-anim-hover 0.7s infinite;
}
._Blesstiny {
	animation: Blesstiny-anim 1s 1;
}
._Blesstiny:hover {
  animation: Blesstiny-anim 1s infinite;
}
._Blade {
  animation: Blade-open .3s 1 ease-in .001s, Blade-anim 2.2s 1 ease .4s;
}
.emote._COGGERS {
    animation: COGGERS-anim 2.4s 1 cubic-bezier(0.15, 0, 0.85, 1);
}
.emote._COGGERS:hover {
    animation: COGGERS-anim-hover 0.8s infinite linear;
}
.emote._COOMER {
  animation: COOMER-anim 0.1s 20, COOMER-color 2s 1;
  transition: filter 0.2s;
}
.emote._COOMER:hover {
  animation: COOMER-anim-hover 0.1s infinite;
  filter: grayscale(1) sepia(.3) saturate(60) hue-rotate(-50deg) brightness(1.8) drop-shadow(-13px 0px 10px rgba(255, 102, 187, 0.25)) drop-shadow(13px 0px 10px rgba(255, 102, 187, 0.25));
}
.emote._Hhhehhehe {
    animation: Hhhehhehe-anim 500ms 1;
    transform-origin: 0 75%;
}
.emote._Hhhehhehe:hover {
    animation: Hhhehhehe-anim-hover 500ms infinite;
}
.emote._JAMSTINY {
    animation: JAMSTINY-anim 0.69s 5 ease-out;
}
.emote._JAMSTINY:hover {
    animation: JAMSTINY-anim-hover 0.69s infinite ease-out;
}
.emote._Nappa {
    animation: Nappa-anim 750ms 1;
}
.emote._Nappa:hover {
    animation: Nappa-anim-hover 750ms infinite;
}
.emote._PepeLaugh {
    animation: PepeLaugh-anim 200ms 3;
}
.emote._PepeLaugh:hover {
    animation: PepeLaugh-anim-hover 200ms infinite;
}
.emote._PepoTurkey {
    animation: PepoTurkey-anim 0.7s 1;
}
.emote._PepoTurkey:hover {
    animation: PepoTurkey-anim-hover 0.7s infinite;
}
.emote._widepeepoHappy {
  animation: widepeepoHappy-anim 1s 1;
}

.emote._widepeepoHappy:hover {
  animation: widepeepoHappy-anim-hover 1s infinite;
}
.emote._REE {
    animation: REE-anim 100ms 7;
}
.emote._REE:hover {
    animation: REE-anim-hover 100ms infinite;
}
.emote._MLADY {
    animation: MLADY-anim 250ms 3;
}
.emote._MLADY:hover {
    animation: MLADY-anim-hover 250ms infinite;
}


`;
(function() {
//
    'use-strict';
    function setStyle () {
        let ifrm = document.querySelector('iframe[src="/embed/chat?follow=%2Fbigscreen"]').contentWindow.document
        let style = ifrm.createElement('style');
        style.id = "myID";
        style.type = 'text/css';
        style.innerHTML = styleinnerHTML;
        ifrm.head.appendChild(style);
    }
     let observer = new MutationObserver((mutations) => {
       mutations.forEach((m) => {
           let o = m.addedNodes[0].lastChild;
           let repStr = o.innerHTML;
           if (repStr.includes(" :") && repStr.includes('class="emote ')) {
               for (let emote of DGG_ANIMATION_EMOTES) {

                   if (repStr.includes(" :" + emote)) {
                       let theIndx = repStr.indexOf('class="emote ', repStr.indexOf(" :" + emote) - 70) + 13;
                       //<div title="OMEGALUL" class="emote OMEGALUL">OMEGALUL </div>
                       repStr = repStr.slice(0, theIndx) + '_' + emote + ' ' + repStr.slice(theIndx); //sets original anim. to new emote anim.
                       repStr = repStr.replace(" :" + emote, ""); //removing :emote suffix
                   }
               }

           }
           o.innerHTML = repStr.replaceAll("KEKW", imgUrl);
       });
    });

    function observeDOM() {
        observer.observe(document.querySelectorAll("iframe")[1].contentWindow.document.querySelector(".chat-lines"), {
            childList: true,
            attributes: true,
            attributeFilter: ['style'],
            subtree: true
        });
    };

    setTimeout(observeDOM, 3000);
    setTimeout(setStyle, 3000);
})();
